---
layout: page
title: "Amplifying Diversity in Polar Exploration"
source: St Catharine's College, University of Cambridge
source_url: https://www.caths.cam.ac.uk/polar-impact
date: 2021-11-22
thumbnail: assets/img/press/cam_catz_polar_impact.jpg
archived_pdf: /assets/pdf/press/2021-11-22-cambridge-catz-polar-impact.pdf
excerpt: A feature by St Catharine's College, Cambridge on my work co-organising Polar Impact, a network amplifying the voices of underrepresented researchers in polar science.
---

{%
  include figure.liquid
  loading="eager"
  path="assets/img/press/cam_catz_polar_impact.jpg"
  alt="Pearl Ayem — St Catharine's College feature on Polar Impact"
  class="img-fluid rounded mb-4"
%}

<p class="text-muted">
  <strong>{{ page.source }}</strong> &middot; {{ page.date | date: "%-d %B %Y" }}
</p>

In November 2021, **St Catharine's College** at the University of Cambridge featured my work as a co-organiser of **Polar Impact**, a volunteer network founded in Cambridge in 2019 to connect and amplify the voices of Black, Asian, Indigenous and minority ethnic professionals and people of colour working in polar research. The full piece is on the St Catharine's website, and below is a recap of the conversation.

## How I found Polar Impact

I'd been following Polar Impact during my undergraduate years in Canada, where the network already had a strong foothold across North America. What drew me in was how a niche, often invisible community could be made visible.

> *"By sharing the real world experiences of our diverse community, we can begin to challenge the stereotype of polar exploration being exclusively the domain of white men. There's still a long way to go but I look forward to Polar Impact expanding its activities into scholarships, training and workshops to support talented researchers from under represented backgrounds."*

## A COP26 social media takeover

One of the first projects I led at Polar Impact was a social media takeover during COP26. From 29 October to 15 November 2021, we handed over the network's accounts to different polar explorers and experts around the world so they could tell their own stories rather than have someone tell those stories for them.

The thing that stayed with me from that project was how varied each person's path into polar research turned out to be. Polar science wasn't a career conversation I ever had with my family growing up in Mumbai or later in Vancouver, and that was true for almost everyone we featured. There isn't a standard route in.

## From computer science to atmospheric science

The interview also touches on my undergraduate years at the University of British Columbia, where I originally applied to study Computer Science before switching to Atmospheric Science.

> *"I worked out pretty quickly that Computer Science was enjoyable to me only when I could apply it to environmental problems. By switching my degree, I was able to use these skills to support climate modelling and research into how the atmosphere is changing. Polar research is going to need people with these skills if we are going to be able to continue to expand our understanding of the habitat and the impact of climate change."*

That intersection of code and climate has been the throughline of everything since. It's also a hopeful one. With the right tools, polar research can become more remote, more collaborative, and a lot less carbon intensive than the discipline has historically been.

> *"Technology also gives us a real shot at establishing Net Zero polar research stations, which currently require field work and long distance travel. My own research uses the data available from sensors and satellite data, which I can process and analyse remotely thanks to the support of the team on the ground in Antarctica."*

## Choosing Cambridge, choosing Catz

Anyone who has been through the Oxbridge applications process knows the collegiate system can be opaque from the outside. I was certain I wanted to apply for the Master's at the Scott Polar Research Institute, but the college side of the decision was less clear.

> *"To be honest, I didn't understand what joining a college meant so I turned to Reddit to find out more. I discovered a thread about the friendliest colleges with lots of recommendations in favour of joining Catz. Fortunately, everything I read proved to be true: everyone has been very welcoming, warm and passionate about their interests."*

## The research itself

For my MPhil, I worked with **Dr Amelie Kirchgaessner** at the British Antarctic Survey and **Professor Ian Willis** at St Catharine's, studying foehn winds on the George VI Ice Shelf, the second largest ice shelf on the Antarctic Peninsula. The question I was trying to answer: how do atmospheric factors aggravate surface melt to the point where an ice shelf can collapse entirely. The work used remote sensor and satellite data, which meant I could analyse a polar system from Cambridge thanks to the team on the ground in Antarctica.

It was the project that crystallised what I want my career to do, which is to use software and data to expand what climate science can see and who gets to do it.

---

- 🔗 **Read the full feature:** [caths.cam.ac.uk → Amplifying diversity in polar exploration]({{ page.source_url }})
{% if page.archived_pdf %}- 📄 **Archived copy:** [download PDF]({{ page.archived_pdf | relative_url }}){% endif %}
